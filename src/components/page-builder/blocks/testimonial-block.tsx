...
export default function TestimonialBlock(props: TestimonialBlockProps) {
    ...
    return (
        <section
            ...
            className={cn(
                "pb-10 md:pb-0 xl:px-10 pattern-bg border-y border-dashed",
                {
                    "rounded-t-4xl": stegaClean(cornerRadiusTop) === "rounded",
                    "rounded-b-4xl":
                        stegaClean(cornerRadiusBottom) === "rounded",
                }
            )}
        >
            <Container className="py-16 md:py-28 space-y-10 border-x border-dashed">
                <div>
                    ...
                    <h2 className="mt-6 py-2 text-center text-xl md:text-2xl font-semibold border-y w-fit mx-auto bg-gradient-to-r from-white/0 via-purple-400/15 to-white/0">
                        {heading}
                    </h2>
                </div>
...
function TestimonialCard({
    ...
    testimonial:
        | NonNullable<TestimonialBlockProps["testimonials"]>[number]
        | null;
    classNames?: string;
}) {
    return (
        <div
            className={cn(
                "h-full mx-auto max-w-[38rem] md:max-w-[44rem] p-8 md:p-12 space-y-10 md:space-y-20 flex flex-col justify-between bg-white",
                classNames
            )}
        >
