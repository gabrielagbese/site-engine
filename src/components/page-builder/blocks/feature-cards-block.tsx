            ...
            <Container className="py-16 md:py-28 px-4 space-y-8 md:space-y-6 border-x border-dashed">
                <div className="relative max-w-[60rem] mx-auto py-2 md:py-4 flex flex-col md:flex-row gap-30 md:gap-6 items-center justify-between border-y border-dashed pattern-bg--2">
                    <Heading
                        tag="h2"
                        size="xl"
                        className="relative col-span-7 py-1.5 text-balance leading-normal"
                    >
                        <span className="relative z-10">{heading}</span>
                    </Heading>
                    {buttons && buttons.length > 0 && (
                        <ButtonRenderer
                            classNames="z-20 hidden md:flex"
                            buttons={buttons}
                        />
                    )}
                    <EdgeBlur />
                </div>
                <div className="max-w-[60rem] mx-auto grid grid-cols-1 md:grid-cols-1 gap-6">
                    ...
                    {showCallToAction && <CallToAction {...props} />}
                </div>
            </Container>
        </section>
    );
...
function FeatureCard({
    ...
    return (
        <div className="border border-dashed rounded-3xl text-center">
            <div className="p-3 flex justify-center">
                <Image
                    ...
                    height={400}
                    alt={feature.title ?? ""}
                    className="rounded-2xl h-[320px] object-cover overflow-hidden"
                />
            </div>
            <div className="mt-5 px-6 md:px-8 pb-2">
                <div className="space-y-6">
                    <Heading
                        tag="h2"
                        size="sm"
                        className="relative py-2 text-3xl font-semibold border-y border-y-gray-200/40 pattern-bg text-center"
                    >
                        {feature.title}
                    </Heading>
                    <p className="text-balance text-gray-500 text-center mx-auto">
                        {feature.description}
                    </p>
                </div>
            </div>
            <div className="mt-4 space-y-3 border-t border-dashed">
                {feature?.items?.map((item, index) => (
                    <div
                        key={item}
                        className={cn(
                            "flex items-center justify-center gap-2 px-6 md:px-8 py-4 border-b border-dashed text-center",
                            {
                                "border-none pb-6":
                                    index === (feature?.items?.length ?? 0) - 1,
                            }
                        )}
                    >
...
function CallToAction(props: FeatureCardsBlockProps) {
    const { callToActionHeading, callToActionContent, callToActionButtons } =
        props;
